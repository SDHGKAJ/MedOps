  ...
  return (
    <div className="medicines-module">
      ...
      {showAddForm && user.role === 'admin' && (
        <form onSubmit={handleAddMedicine} style={{ background: '#f8fafc', padding: '1.5rem', borderRadius: '8px', marginBottom: '2rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
            <input required name="name" placeholder="Name" value={formData.name} onChange={handleChange} className="form-input" style={{ width: '100%', boxSizing: 'border-box' }} />
            <input required name="manufacturer" placeholder="Manufacturer" value={formData.manufacturer} onChange={handleChange} className="form-input" style={{ width: '100%', boxSizing: 'border-box' }} />
            <input required name="category" placeholder="Category" value={formData.category} onChange={handleChange} className="form-input" style={{ width: '100%', boxSizing: 'border-box' }} />
            <input required name="price" type="number" placeholder="Price" value={formData.price} onChange={handleChange} className="form-input" style={{ width: '100%', boxSizing: 'border-box' }} />
            <input required name="quantity" type="number" placeholder="Quantity" value={formData.quantity} onChange={handleChange} className="form-input" style={{ width: '100%', boxSizing: 'border-box' }} />
            <input required name="description" placeholder="Description" value={formData.description} onChange={handleChange} className="form-input" style={{ width: '100%', boxSizing: 'border-box' }} />
          </div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1rem', maxHeight: 'calc(100vh - 300px)', overflowY: 'auto', paddingRight: '0.5rem', paddingBottom: '1rem' }}>
          {medicines.length === 0 ? <p>No medicines found.</p> : medicines.map(med => (
            <div key={med._id} style={{ border: '1px solid #000', padding: '1rem', borderRadius: '8px', background: 'white' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <h3 style={{ margin: '0 0 0.5rem 0', color: '#1e293b' }}>{med.name}</h3>
                <span style={{ background: med.quantity > 0 ? '#c7ccc9ff' : '#fee2e2', color: med.quantity > 0 ? '#166534' : '#991b1b', padding: '0.25rem 0.5rem', borderRadius: '4px', fontSize: '0.875rem', fontWeight: 'bold' }}>
